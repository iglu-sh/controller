...
export class Logger{
    ...
    private static prefix: string = chalk.bgBlueBright('SCHEDULER');
    ...
    static log(level:LogLevel, message: string){
        ...
        if(Logger.jsonLogging){
            ...
            console.log(Logger.customChalk.grey(`${new Date().toISOString()}`), Logger.prefix, Logger.logLevelColorMap[level](`${level}`), message);
        }
    }
    ...
    static logRequest(endpoint:string, method:string){
        if(Logger.jsonLogging){
            ...
            console.log(Logger.customChalk.grey(`${new Date().toISOString()}`), `${Logger.verbColorMap[method](method)} ${Logger.prefix}`, endpoint);
        }
    }
    static logResponse(endpoint:string, method:string, statusCode:number){
        if(Logger.jsonLogging){
            ...
            console.log(Logger.customChalk.grey(`${new Date().toISOString()}`), statusCodeColor(statusCode), `${Logger.verbColorMap[method](method)} ${Logger.prefix}`, endpoint);
        }
    }
    constructor() {
        dotenv.config()
