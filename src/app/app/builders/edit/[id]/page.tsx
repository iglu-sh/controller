...
async function createBuilderRequest(data: BuilderCreationRequest, cache?:string, setBuilder: (builder: builderDatabaseRepresenation) => void) {
    ...
    if(typeof data.build.maxJobs !== "number" || typeof data.build.cores !== "number") {
        // Cast these two values to numbers
        try{
            data.build.maxJobs = parseInt(data.build.maxJobs as unknown as string, 10);
            data.build.cores = parseInt(data.build.cores as unknown as string, 10);
        }
        catch(e){
            toast.error("Max Jobs and Cores must be positive integers.");
            return;
        }
    }
            ...
            <Card>
                ...
                <CardContent>
                    <Tabs defaultValue="general" className="w-full">
                        ...
                        <TabsContent value="cachix"><Cachix data={data} targetCache={targetCache} setData={(data)=>setData(data)} setTargetCache={(cache:string)=>setTargetCache(cache)}/></TabsContent>
